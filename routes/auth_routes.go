...
func SetupAuthRoutes(e *echo.Echo) {
	...
	e.GET("/user", func(c echo.Context) error {
		session, _ := auth.SessionStore.Get(c.Request(), "auth-session")
...
func loginHandler(c echo.Context) error {
	...
	return c.Redirect(http.StatusTemporaryRedirect, auth.Auth.AuthCodeURL(state))
}
...
func callbackHandler(c echo.Context) error {
	...
	token, err := auth.Auth.Exchange(c.Request().Context(), c.QueryParam("code"))
	if err != nil {
		return c.String(http.StatusUnauthorized, "Failed to exchange code for token")
	}

	idToken, err := auth.Auth.VerifyIDToken(c.Request().Context(), token)
	if err != nil {
